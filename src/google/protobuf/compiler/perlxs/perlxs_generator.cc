...
bool PerlXSGenerator::Generate(const FileDescriptor* file,
...
const std::string& PerlXSGenerator::GetVersionInfo() const {
  return PackageString;
}

bool PerlXSGenerator::ProcessOption(const std::string& option) {
  size_t equals;
...
void PerlXSGenerator::GenerateMessageXS(const Descriptor* descriptor,
...
void PerlXSGenerator::GenerateMessageModule(const Descriptor* descriptor,
                                            OutputDirectory* outdir) const {
...
void PerlXSGenerator::GenerateMessagePOD(const Descriptor* descriptor,
                                         OutputDirectory* outdir) const {
...
void PerlXSGenerator::GenerateDescriptorClassNamePOD(
    const Descriptor* descriptor, io::Printer& printer) const {
  for (int i = 0; i < descriptor->enum_type_count(); i++) {
    printer.Print(
        ...
        "\n",
        "name", EnumClassName(descriptor->enum_type(i)), "enum",
        descriptor->enum_type(i)->full_name());
  }
  ...
  printer.Print(
      ...
      "\n",
      "name", MessageClassName(descriptor), "message", descriptor->full_name());
}

void PerlXSGenerator::GenerateDescriptorMethodPOD(const Descriptor* descriptor,
...
void PerlXSGenerator::GenerateEnumModule(const EnumDescriptor* enum_descriptor,
  ...
  for (int i = 0; i < enum_descriptor->value_count(); i++) {
    ...
    printer.Print("use constant *value* => *number*;\n", "value",
                  enum_descriptor->value(i)->name(), "number", ost.str());
  }
  ...
  for (int i = 0; i < enum_descriptor->value_count(); i++) {
    printer.Print(" my $*value* = *package*::*value*;\n", "package",
                  vars["package"], "value", enum_descriptor->value(i)->name());
  }
...
void PerlXSGenerator::GenerateFileXSTypedefs(
...
void PerlXSGenerator::GenerateMessageXSTypedefs(
...
void PerlXSGenerator::GenerateMessageStatics(const Descriptor* descriptor,
                                             io::Printer& printer) const {
  for (int i = 0; i < descriptor->nested_type_count(); i++) {
...
void PerlXSGenerator::GenerateMessageXSFieldAccessors(
    const FieldDescriptor* field, io::Printer& printer,
  ...
  switch (fieldtype) {
    ...
    case FieldDescriptor::CPPTYPE_STRING:
      ...
      if (type == FieldDescriptor::TYPE_STRING) {
        printer.Print(vars, "    std::string $value$;\n");
      }
      printer.Print(
          "\n"
          "  CODE:\n");
  ...
  if (repeated) {
    if (fieldtype == FieldDescriptor::CPPTYPE_MESSAGE) {
      ...
      if (type == FieldDescriptor::TYPE_BYTES) {
        printer.Print(vars, "    THIS->add_$cppname$(str, len);\n");
      ...
      printer.Print(vars, "    THIS->add_$cppname$($value$);\n");
    }
  } else {
    if (fieldtype == FieldDescriptor::CPPTYPE_MESSAGE) {
      printer.Print(vars,
      ...
      if (type == FieldDescriptor::TYPE_STRING) {
        ...
        printer.Print(vars, "    THIS->set_$cppname$(str, len);\n");
      } else {
        // Can't get here
      }
    } else {
      printer.Print(vars, "    THIS->set_$cppname$($value$);\n");
    }
  }
...
void PerlXSGenerator::GenerateMessageXSCommonMethods(
    const Descriptor* descriptor, io::Printer& printer,
    const std::string& classname) const {
  ...
  for (int i = 0; i < descriptor->field_count(); i++) {
    ...
    vars["field"] = field->name();
    printer.Print(vars, "    PUSHs(sv_2mortal(newSVpv(\"$field$\",0)));\n");
  }
...
void PerlXSGenerator::GenerateMessageXSPackage(const Descriptor* descriptor,
                                               io::Printer& printer) const {
  for (int i = 0; i < descriptor->nested_type_count(); i++) {
...
void PerlXSGenerator::GenerateTypemapInput(const Descriptor* descriptor,
                                           io::Printer& printer,
...
static std::string QualifiedFileLevelSymbol(const FileDescriptor* file,
...
std::string PerlXSGenerator::QualifiedClassName(const Descriptor* d) const {
  return QualifiedFileLevelSymbol(d->file(), cpp::ClassName(d));
}

std::string PerlXSGenerator::QualifiedClassName(const EnumDescriptor* d) const {
  return QualifiedFileLevelSymbol(d->file(), cpp::ClassName(d));
}

// Returns the containing Perl module name for a message descriptor.

std::string PerlXSGenerator::MessageModuleName(
...
std::string PerlXSGenerator::MessageClassName(
    const Descriptor* descriptor) const {
...
std::string PerlXSGenerator::EnumClassName(
...
std::string PerlXSGenerator::PackageName(const std::string& name,
...
void PerlXSGenerator::PerlSVGetHelper(io::Printer& printer,
...
void PerlXSGenerator::PODPrintEnumValue(const EnumValueDescriptor* value,
                                        io::Printer& printer) const {
  std::ostringstream ost;
  printer.Print(
      "=item B<*value*>\n"
...
std::string PerlXSGenerator::PODFieldTypeString(
...
void PerlXSGenerator::StartFieldToHashref(
    const FieldDescriptor* field, io::Printer& printer,
...
void PerlXSGenerator::EndFieldToHashref(
    const FieldDescriptor* field, io::Printer& printer,
    std::map<std::string, std::string>& vars, int depth) const {
  vars["field"] = field->name();
...
void PerlXSGenerator::MessageToHashref(const Descriptor* descriptor,
                                       io::Printer& printer,
                                       std::map<std::string, std::string>& vars,
                                       int depth) const {
  for (int i = 0; i < descriptor->field_count(); i++) {
...
void PerlXSGenerator::FieldFromHashrefHelper(
    io::Printer& printer, std::map<std::string, std::string>& vars,
    const FieldDescriptor* field) const {
  vars["msg"] = "msg" + vars["pdepth"];
  ...
  switch (field->cpp_type()) {
    ...
    case FieldDescriptor::CPPTYPE_STRING:
      ...
      if (field->type() == FieldDescriptor::TYPE_STRING) {
        ...
        printer.Print(vars, "$msg$->$do$_$cppname$(str, len);\n");
      } else {
        // Can't get here
      }
      break;
